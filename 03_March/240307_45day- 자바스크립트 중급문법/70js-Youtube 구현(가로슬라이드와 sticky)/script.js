...
const getTranslateX = () => {
  ...
  return parseInt(
    window.getComputedStyle(hashContent).transform.split(/[^\-0-9]+/g)
  )[5];
};
...
const onScrollEnd = (e) => {
  ...
  window.removeEventListener("touchend", onScrollStart);
  window.removeEventListener("mousedown", onScrollMove);
  window.removeEventListener("touchend", onScrollMove);
  window.removeEventListener("mousedown", onScrollEnd);
